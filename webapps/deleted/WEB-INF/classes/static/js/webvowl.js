...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = function ( graphContainerSelector ){
	  ...
	  graph.start = function (){
		...
		// setTimeout(function(){ graph.locateSearchResult();	 }, 0);
		// setTimeout(function(){ 		console.log("pausedin");		graph.paused(true);		}, 15000);
		// setTimeout(function(){ 		console.log("search");		graph.locateSearchResult();	}, 15001);
		// setTimeout(function(){ 		console.log("highlight");		graph.highLightNodes(["96"]);	}, 15001);
	  ...
	  graph.updateStyle = function (){
	    ...
	    if ( graph.options().loadingModule().successfullyLoadedOntology() === false ) {
	      ...
	      force.start();
				...
				// setTimeout(function(){ graph.locateSearchResult();	graph.highLightNodes(); }, 0);
	  ...
	  graph.update = function ( init ){
	    ...
	    refreshGraphStyle();
		updateHaloStyles();
		// setTimeout(function(){ 	   targetLocationZoom(force.nodes()[136]);		}, 10000);
	  ...
	  graph.paused = function ( p ){
		if ( !arguments.length ){
		console.log("paused"); 
		setTimeout(function(){ 		console.log("pausedin");		graph.paused(true);		}, 2000);
		setTimeout(function(){ 		console.log("search");		graph.locateSearchResult();	}, 2001);
		setTimeout(function(){ 		console.log("highlight");		graph.highLightNodes(["284"]);	}, 2002);
		setTimeout(function(){ 		console.log("highlight");		graph.highLightNodes(["427"]);	}, 2002);

		return paused;
	}
	    paused = p;
		graph.updateStyle();
		console.log("paused2"+p);
	  ...
	  function loadGraphData( init ){
	    ...
	    if ( graphContainer && validOntology === true ) {
	      ...
	      updateRenderingDuringSimulation = false;
	      graph.options().ontologyMenu().append_bulletPoint("온톨로지 시각화 중 ... ");
	      loadingModule.setPercentMode();
